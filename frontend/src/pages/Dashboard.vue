  ...
  <div class="page">
    ...
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <button
          v-if="activeTab === 'dashboard' && (widgets.length > 0 || stats.totalOrders > 0)"
          class="btn-configure"
          @click="$router.push('/dashboard-config')"
        >
          ⚙ Configure dashboard
        </button>
      </div>
    ...
    <div v-show="activeTab === 'dashboard'">
      ...
      <div v-else class="widgets-grid">
        ...
        >
          ...
          <!-- Chart Widgets -->
          <div v-else-if="['bar','line','area','scatter'].includes(widget.type)" class="widget-preview chart-widget">
    ...
    <div v-show="activeTab === 'table'">
      ...
      <div v-if="orders.length === 0" class="dashboard-empty-state">
        ...
        <button class="btn-configure-center" @click="openCreateModal">+ Create order</button>
      </div>
      ...
      <div class="modal">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input v-model="form.phone" class="form-input" :class="{ error: errors.phone }" type="text" placeholder="+1-234-567-8900"/>
          ...
          <div class="form-group full-width">
            ...
            <span v-if="errors.street_address" class="error-message">{{ errors.street_address }}</span>
          </div>
          ...
          <div class="form-group full-width">
            ...
            <span v-if="errors.created_by" class="error-message">{{ errors.created_by }}</span>
          </div>
...
<script>
import { getDashboard, getOrders, createOrder, updateOrder, deleteOrder, getWidgetData } from '../services/api.js'
...
export default {
  ...
  methods: {
    ...
    async loadStats() {
      try {
        ...
        const [ordersRes, revenueRes, quantityRes] = await Promise.all([
          getWidgetData({ type: 'kpi', metric: 'total_amount', aggregation: 'Count', ...params }),
          getWidgetData({ type: 'kpi', metric: 'total_amount', aggregation: 'Sum', ...params }),
          getWidgetData({ type: 'kpi', metric: 'quantity', aggregation: 'Sum', ...params })
        ])
        ...
        try {
          if (widget.type === 'kpi' && widget.metric) {
            const res = await getWidgetData({
              type: 'kpi', metric: widget.metric,
              aggregation: widget.aggregation, dateRange: this.dateRange
            })
            this.widgetData[widget.id] = res.data.value
          }
          if (['bar','line','area','scatter'].includes(widget.type) && widget.xAxis && widget.yAxis) {
            const res = await getWidgetData({
              type: widget.type, xAxis: widget.xAxis,
              yAxis: widget.yAxis, dateRange: this.dateRange
            })
            await this.$nextTick()
          ...
          if (widget.type === 'pie' && widget.chartData) {
            const res = await getWidgetData({
              type: 'pie', chartData: widget.chartData,
          ...
          if (widget.type === 'table') {
            const res = await getWidgetData({ type: 'table', dateRange: this.dateRange })
    ...
    renderChart(widget, data) {
      ...
      const getBackground = (ctx) => {
        if (widget.type === 'bar') return color + 'dd'
        if (widget.type === 'area') {
      ...
      this.charts[widget.id] = new Chart(canvas, {
        ...
        data: {
          ...
          datasets: [{
            ...
            pointBorderWidth: 2,
            pointRadius: ['line','area'].includes(widget.type) ? 5 : 0,
            pointHoverRadius: 7
          }]
        },
        options: {
          ...
          scales: {
            x: {
              ticks: { font: { size: 11 }, color: '#94a3b8', maxRotation: 0, minRotation: 0 },
              grid: { display: false },
    ...
    renderPieChart(widget, data) {
      ...
      const values = data.map(d => parseFloat(d.value) || 0)
      const colors = ['#0d9488','#3b82f6','#f59e0b','#ef4444','#8b5cf6','#10b981','#f97316','#06b6d4']
    ...
    getTableColumns(widget) {
      ...
      const labels = {
        ...
        order_id: 'Order ID', order_date: 'Order date', product: 'Product',
        quantity: 'Quantity', unit_price: 'Unit price',
        total_amount: 'Total amount', status: 'Status', created_by: 'Created by'
      }
    ...
    getTableRows(widget) {
      ...
      return data.slice(0, limit).map(row => ({
        ...
        order_date: row.order_date
          ? new Date(row.order_date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })
          : ''
      }))
    },
    ...
    validateForm() {
      ...
      } else if (!/^[a-zA-Z\s]+$/.test(this.form.first_name)) {
        this.errors.first_name = 'Only letters allowed'
      }

      if (!this.form.last_name) {
        this.errors.last_name = 'This field is required'
      } else if (!/^[a-zA-Z\s]+$/.test(this.form.last_name)) {
        this.errors.last_name = 'Only letters allowed'
      }
      ...
      if (!this.form.phone) {
        ...
        this.errors.phone = 'Numbers only — e.g. +1-234-567-8900'
      } else if (this.form.phone.replace(/\D/g, '').length < 7) {
        this.errors.phone = 'Phone number too short'
      }

      // Address fields
      if (!this.form.street_address) this.errors.street_address = 'This field is required'

      if (!this.form.city) {
        this.errors.city = 'This field is required'
      } else if (!/^[a-zA-Z\s]+$/.test(this.form.city)) {
        this.errors.city = 'Only letters allowed'
      }

      if (!this.form.state) {
        this.errors.state = 'This field is required'
      } else if (!/^[a-zA-Z\s]+$/.test(this.form.state)) {
        this.errors.state = 'Only letters allowed'
      }

      if (!this.form.postal_code) {
        this.errors.postal_code = 'This field is required'
      } else if (!/^[a-zA-Z0-9\s\-]+$/.test(this.form.postal_code)) {
        this.errors.postal_code = 'Invalid postal code'
      }

      // Dropdowns
      if (!this.form.country) this.errors.country = 'Please select a country'
      if (!this.form.product) this.errors.product = 'Please select a product'
      if (!this.form.created_by) this.errors.created_by = 'Please select a person'

      // Quantity
      if (!this.form.quantity || this.form.quantity < 1) {
        this.errors.quantity = 'Quantity must be at least 1'
      } else if (!Number.isInteger(Number(this.form.quantity))) {
        this.errors.quantity = 'Must be a whole number'
      }

      // Unit price
      if (this.form.unit_price === '' || this.form.unit_price === null || this.form.unit_price === undefined) {
        this.errors.unit_price = 'This field is required'
      } else if (isNaN(this.form.unit_price) || this.form.unit_price < 0) {
        this.errors.unit_price = 'Enter a valid price'
      }
